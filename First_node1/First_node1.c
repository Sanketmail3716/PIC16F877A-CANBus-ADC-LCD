...
char RxTx_Data[8];
char Msg_Rcvd;
const long ID_1st = 12111, ID_2nd = 11121;//ID_3nd = 4;
long Rx_ID;
...
void main() {
    ...
    while(1){
        adc_value_1 = ADC_Read(0); // 10-bit ADC

        RxTx_Data[0] = adc_value_1 >> 8;     // MSB
        RxTx_Data[1] = adc_value_1 & 0xFF;   // LSB
        Lcd_Out(1,1,"Node2:");
        Lcd_Cmd(0x86);
        ...
        Lcd_Out(1,11,"     ");
        CANSPIWrite(ID_1st, RxTx_Data, 2, Can_Send_Flags); // Send to Node B
        Delay_ms(100);
          adc_value_2 = ADC_Read(3); // 10-bit ADC

        RxTx_Data[0] = adc_value_2 >> 8;     // MSB
        RxTx_Data[1] = adc_value_2 & 0xFF;   // LSB
        Lcd_Out(2,1,"Node1:");
        Lcd_Cmd(0xC6);
        ...
        Lcd_Out(1,11,"     ");
        CANSPIWrite(ID_2nd, RxTx_Data, 2, Can_Send_Flags); // Send to Node B
        Delay_ms(100);
    }
}